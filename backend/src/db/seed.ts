...
async function main() {
  ...
  // --- Wards ---
  try {
    const wardRows = await db.insert(wards).values([
    ...
    console.log("Wards inserted:", wardRows.length);
  } catch (err) {
  ...
  try {
    ...
    console.log("Wards fetched from DB:", allWards.length);
    ...
    const aliasesToInsert: { wardId: string; areaName: string }[] = [];
    for (const group of aliasData) {
      ...
      if (wId) {
        ...
        console.warn("WARNING: No ward found for alias group:", group.name);
      }
    }

    console.log("Aliases to insert (prepared):", aliasesToInsert.length);

    if (aliasesToInsert.length > 0) {
      const aliasRows = await db.insert(ward_aliases).values(aliasesToInsert).onConflictDoNothing().returning();
      console.log("Ward aliases inserted:", aliasRows.length);
    } else {
      console.warn("WARNING: No aliases to insert — aliasesToInsert is empty!");
    }
  } catch (err) {
    console.error("ERROR inserting ward aliases:", err);
  }
